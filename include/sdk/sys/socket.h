...
#define SOCK_STREAM                                                                                                    \
    1 /* Provides sequenced, reliable, two-way,                                                                        \
       * connection-based byte streams. An out-of-band data                                                            \
       * transmission mechanism may be supported.                                                                      \
       */
#define SOCK_DGRAM                                                                                                     \
    2              /* Supports  datagrams (connectionless, unreliable                                                  \
                    * messages of a fixed maximum length).                                                             \
                    */
#define SOCK_RAW 3 /* Provides raw network protocol access. */
#define SOCK_RDM                                                                                                       \
    4 /* Provides a reliable datagram layer that does not                                                              \
       * guarantee ordering.                                                                                           \
       */
#define SOCK_SEQPACKET                                                                                                 \
    5 /* Provides a sequenced, reliable, two-way                                                                       \
...
#define SOCK_TYPE_MASK                                                                                                 \
    0xf /* Mask which covers at least up to SOCK_MASK-1.                                                               \
...
#define MSG_CMSG_CLOEXEC                                                                                               \
    0x100000 /* Set close_on_exit for file                                                                             \
...
#define SHUT_RDWR                                                                                                      \
    3 /* Bits 0+1: Disables further send and receive                                                                   \
       * operations                                                                                                    \
       */
...
#define __CMSG_FIRSTHDR(ctl, len) ((len) >= sizeof(struct cmsghdr) ? (struct cmsghdr *)(ctl) : NULL)
...
#define SS_ALIGNSIZE (sizeof(struct sockaddr *))
...
static inline struct cmsghdr *__cmsg_nxthdr(void *__ctl, unsigned int __size, struct cmsghdr *__cmsg) {
    ...
    struct cmsghdr *__ptr = (struct cmsghdr *)(((char *)__cmsg) + len);

    if (len < sizeof(*__cmsg) || (unsigned long)((char *)(__ptr + 1) - (char *)__ctl) > __size) {
        return NULL;
    }
