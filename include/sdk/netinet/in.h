...
#define IP_MULTICAST_IF                                                                                                \
    (__SO_PROTOCOL + 1) /* Set local device for a                                                                      \
                         * multicast socket */
#define IP_MULTICAST_TTL                                                                                               \
    (__SO_PROTOCOL + 2) /* Set/read the time-to-                                                                       \
                         * live value of outgoing                                                                      \
                         * multicast packets */
#define IP_MULTICAST_LOOP                                                                                              \
    (__SO_PROTOCOL + 3)                        /* Set/read boolean that                                                \
...
#define IP_UNBLOCK_SOURCE                                                                                              \
    (__SO_PROTOCOL + 6) /* Unblock previously                                                                          \
                         * blocked multicast source */
#define IP_BLOCK_SOURCE                                                                                                \
    (__SO_PROTOCOL + 7) /* Stop receiving multicast                                                                    \
                         * data from source */

#define IP_ADD_SOURCE_MEMBERSHIP                                                                                       \
    (__SO_PROTOCOL + 8) /* Join a multicast group;                                                                     \
                         * allow receive only from                                                                     \
                         * source */

#define IP_DROP_SOURCE_MEMBERSHIP                                                                                      \
    (__SO_PROTOCOL + 9) /* Leave a source-specific                                                                     \
...
#define IP_MSFILTER                                                                                                    \
    (__SO_PROTOCOL + 10) /* Access advanced, full-                                                                     \
                          * state filtering API */
#define IP_MULTICAST_ALL                                                                                               \
    (__SO_PROTOCOL + 11) /* Modify the delivery policy                                                                 \
                          * of multicast messages bound                                                                \
                          * to INADDR_ANY */
#define IP_PKTINFO                                                                                                     \
    (__SO_PROTOCOL + 12) /* Get some information about                                                                 \
                          * the incoming packet */
#define IP_TOS                                                                                                         \
    (__SO_PROTOCOL + 13) /* Access the Type-Of-Service                                                                 \
                          * (TOS) field  */
#define IP_TTL                                                                                                         \
    (__SO_PROTOCOL + 14) /* The IP TTL (time to live)                                                                  \
...
#define IPV6_MULTICAST_IF                                                                                              \
    (__SO_PROTOCOL + 4) /* Interface to use for                                                                        \
                         * outgoing multicast packets */
#define IPV6_MULTICAST_LOOP                                                                                            \
    (__SO_PROTOCOL + 5)                       /* Multicast packets are                                                 \
                                               * delivered back to the local                                           \
                                               * application */
#define IPV6_UNICAST_HOPS (__SO_PROTOCOL + 6) /* Unicast hop limit */
#define IPV6_V6ONLY                                                                                                    \
    (__SO_PROTOCOL + 7) /* Restrict AF_INET6 socket                                                                    \
                         * to IPv6 communications only */
#define IPV6_PKTINFO                                                                                                   \
    (__SO_PROTOCOL + 8) /* Information about the                                                                       \
                         * incoming packet */
#define IPV6_RECVPKTINFO                                                                                               \
    (__SO_PROTOCOL + 9) /* Receive the information about                                                               \
                         * the incoming packet */
#define IPV6_TCLASS                                                                                                    \
    (__SO_PROTOCOL + 10)                       /* Access the Traffic Class                                             \
                                                * field */
...
#define INADDR_ANY                                                                                                     \
    ((in_addr_t)0x00000000) /* Address to accept any                                                                   \
                             * incoming messages */
#define INADDR_BROADCAST                                                                                               \
    ((in_addr_t)0xffffffff) /* Address to send to                                                                      \
                             * all hosts */
#define INADDR_NONE                                                                                                    \
    ((in_addr_t)0xffffffff)                     /* Address indicating an                                               \
                                                 * error return */
...
#define IN6_IS_ADDR_LINKLOCAL(a) ((a)->s6_addr16[0] & HTONS(0xffc0) == HTONS(0xfe80))

#define IN6_IS_ADDR_LOOPBACK(a)                                                                                        \
    ((a)->s6_addr32[0] == 0 && (a)->s6_addr32[1] == 0 && (a)->s6_addr32[2] == 0 && (a)->s6_addr32[3] == HTONL(1))
...
#define IN6_IS_ADDR_V4MAPPED(a) ((a)->s6_addr32[0] == 0 && (a)->s6_addr32[1] == 0 && (a)->s6_addr32[2] == HTONL(0xffff))
