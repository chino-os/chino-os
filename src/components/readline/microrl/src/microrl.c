...
static int split(microrl_t *pThis, int limit, char const **tkn_arr) {
    ...
    int ind = 0;
    while (1) {
        if (ind < limit && pThis->cmdline[ind] == ' ')
            pThis->cmdline[ind] = '\0';
...
//*****************************************************************************
inline static void print_prompt(microrl_t *pThis) { printf(pThis->prompt_str); }

//*****************************************************************************
inline static void terminal_backspace(microrl_t *pThis) { printf("\033[D \033[D"); }

//*****************************************************************************
inline static void terminal_newline(microrl_t *pThis) { printf(ENDL); }

void microrl_prompt(microrl_t *pThis) { print_prompt(pThis); }
...
static void terminal_reset_cursor(microrl_t *pThis) {
    char str[16];
#ifdef _USE_LIBC_STDIO
    snprintf(str, 16, "\033[%dD\033[%dC", _COMMAND_LINE_LEN + pThis->prompt_len + 2, pThis->prompt_len);
...
static void terminal_print_line(microrl_t *pThis, int pos, int cursor) {
    printf("\033[K"); // delete all from cursor to end
...
void microrl_init(microrl_t *pThis) {
    ...
    pThis->prompt_len = _DEFAULT_PROMPT_LEN;
...
void microrl_set_newline_callback(microrl_t *pThis, int (*newline)(const char *)) { pThis->newline = newline; }
#ifdef _USE_CTLR_C
//*****************************************************************************
void microrl_set_sigint_callback(microrl_t *pThis, void (*sigintf)(void)) { pThis->sigint = sigintf; }
#endif
...
static int microrl_insert_text(microrl_t *pThis, char *text, int len) {
    ...
    if (pThis->cmdlen + len < _COMMAND_LINE_LEN) {
        memmove(pThis->cmdline + pThis->cursor + len, pThis->cmdline + pThis->cursor, pThis->cmdlen - pThis->cursor);
        for (i = 0; i < len; i++) {
...
static void microrl_backspace(microrl_t *pThis) {
    if (pThis->cursor > 0) {
        ...
        memmove(pThis->cmdline + pThis->cursor - 1, pThis->cmdline + pThis->cursor, pThis->cmdlen - pThis->cursor + 1);
        pThis->cursor--;
        pThis->cmdline[pThis->cmdlen] = '\0';
        pThis->cmdlen--;
    }
...
static void microrl_get_complite(microrl_t *pThis) {
    ...
    if (compl_token[0] != NULL) {
        ...
        if (len) {
            microrl_insert_text(pThis, compl_token[0] + strlen(tkn_arr[status - 1]), len - strlen(tkn_arr[status - 1]));
            if (compl_token[1] == NULL)
...
int new_line_handler(microrl_t *pThis) {
    ...
    if (pThis->wait_newline) {
        ...
        if (status == -1) {
            //          printf ("ERROR: Max token amount exseed\n");
            printf("ERROR:too many tokens");
            printf(ENDL);
        }
        if ((status > 0) && (pThis->execute != NULL))
            bexit = pThis->execute(status, tkn_arr);
    }
