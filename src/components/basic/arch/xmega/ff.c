...
#define INIT_NAMBUF(dj)                                                                                                \
    {                                                                                                                  \
        ...
        if (!lfn)                                                                                                      \
            LEAVE_FF((dj).obj.fs, FR_NOT_ENOUGH_CORE);                                                                 \
        (dj).fn = sfn;                                                                                                 \
        (dj).lfn = lfn;                                                                                                \
        (dj).obj.fs->dirbuf = (BYTE *)(lfn + _MAX_LFN + 1);                                                            \
    }
#define FREE_NAMBUF() ff_memfree(lfn)
#define DEF_DIRBUF BYTE *dirb
#define INIT_DIRBUF(fs)                                                                                                \
    {                                                                                                                  \
        dirb = ff_memalloc(SZDIRE * 19);                                                                               \
        if (!dirb)                                                                                                     \
    ...
    if (fs->fs_type == FS_EXFAT) { /* At the exFAT */
        ...
        while ((res = dir_read(dp, 0)) == FR_OK) { /* Read an item */
            ...
            for (nc = fs->dirbuf[XDIR_NumName], di = SZDIRE * 2, ni = 0; nc;
                ...
                if ((di % SZDIRE) == 0)
                    di += 2;
                if (ff_wtoupper(ld_word(fs->dirbuf + di)) != ff_wtoupper(dp->lfn[ni]))
                    break;
            }
            if (nc == 0 && !dp->lfn[ni])
                break; /* Name matched? */
...
dir_register(        /* FR_OK:succeeded, FR_DENIED:no free entry or too many SFN collision, FR_DISK_ERR:disk error */
    ...
    fn = dp->fn;
    lfn = dp->lfn;
    if (fn[NSFLAG] & (NS_DOT | NS_NONAME))
        return FR_INVALID_NAME; /* Check name validity */
    for (nlen = 0; lfn[nlen]; nlen++)
        ; /* Get lfn length */
        ...
        for (;;) {
            if (*pp == '?' || *pp == '*') { /* Wildcard? */
                ...
                if (pattern_matching(pp, np, nm, nx))
                    return 1; /* Test new branch (recursions upto number of wildcard blocks in the pattern) */
                nc = *np;
...
static BYTE check_fs(           /* 0:FAT, 1:exFAT, 2:Valid BS but not FAT, 3:Not a BS, 4:Disk error */
    ...
    if ((ld_dword(&fs->win[BS_FilSysType32]) & 0xFFFFFF) == 0x544146)
        return 0; /* Check "FAT" string */
#if _FS_EXFAT
    if (!mem_cmp(&fs->win[BS_OEMName], "EXFAT   ", 8))
        return 1;
#endif
    return 2;
}
...
static FRESULT find_volume(                    /* FR_OK(0): successful, !=0: any error occurred */
    ...
    *rfs = fs;    /* Return pointer to the file system object */
    ...
    if (fmt == 1) {
        ...
        for (i = BPB_ZeroedEx; i < BPB_ZeroedEx + 53 && !fs->win[i]; i++)
            ; /* Check zero filler */
        ...
        if (fs->n_fats != 1)
            return FR_NO_FILESYSTEM; /* (Must be 1) */
        ...
        fs->n_fatent = nclst + 2;
        if (fs->n_fatent >= 0x80000000)
            return FR_NO_FILESYSTEM; /* (Must be <= 0x7FFFFFFF) */

        /* Boundaries and Limits */
        fs->volbase = bsect;
        fs->database = bsect + ld_dword(fs->win + BPB_DataOfsEx);
        fs->fatbase = bsect + ld_dword(fs->win + BPB_FatOfsEx);
        if (maxlba < fs->database + nclst * fs->csize)
    ...
    for (; btw; /* Repeat until all data written */
        ...
        if ((fp->fptr % SS(fs)) == 0) {                          /* On the sector boundary? */
            ...
            if (fp->sect != sect) { /* Fill sector cache with file data */
                if (fp->fptr < fp->obj.objsize && disk_read(fs->drv, fp->buf, sect, 1) != RES_OK) {
                    ABORT(fs, FR_DISK_ERR);
                }
            }
#endif
    ...
    if (res == FR_OK) {
        ...
        if (res == FR_OK) { /* Object to be renamed is found */
            ...
            if (fs->fs_type == FS_EXFAT) { /* At exFAT */
                ...
                if (res == FR_OK)
                    res = FR_EXIST;           /* Is new name already in use? */
...
#endif
    {
        ...
        for (;;) { /* Find a contiguous cluster block */
            ...
            if (val == 0) { /* Is it a free cluster? */
                if (++ncl == tcl)
                    break; /* Break if a contiguous cluster block was found */
            } else {
                scl = clst;
